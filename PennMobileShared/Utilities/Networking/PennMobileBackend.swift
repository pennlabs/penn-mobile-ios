...
public struct PennMobileBackend {
    ...
    public static func executeEndpoint<T: PennMobileEndpoint>(_ endpoint: T, toast presentToast: @escaping ToastPresentationManager) async -> BackendFetchedResult<T.Response> {
        ...
        do {
            ...
            guard let http = res as? HTTPURLResponse else {
                throw BackendError.unknownResponseError(rawResponse: res)
            }
            ...
            return .success(try dec.decode(T.Response.self, from: data))
        } catch let error as PlatformError {
            Task { @MainActor in
                presentToast(endpoint.errorResponse(error: .platformError(error: error)))
            }
            return .failure(.platformError(error: error))
            
        } catch let error as BackendError {
            Task { @MainActor in
                presentToast(endpoint.errorResponse(error: error))
            }
            return .failure(error)
        } catch let error as DecodingError {
            Task { @MainActor in
                presentToast(endpoint.errorResponse(error: .decodingError(error: error)))
            }
            return .failure(.decodingError(error: error))
        } catch {
            Task { @MainActor in
                presentToast(endpoint.errorResponse(error: .otherError(error: error)))
            }
            return .failure(.otherError(error: error))
        }
    }
}
