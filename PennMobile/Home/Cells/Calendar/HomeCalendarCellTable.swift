...
final class HomeCalendarCellTable: UITableViewCell, HomeCellConformable {
    var cardView: UIView! = UIView()
    
    var delegate: ModularTableViewCellDelegate!
    
    static var identifier: String = "calendarCell2"
    
    var item: ModularTableViewItem! {
        didSet {
            guard let item = item as? HomeCalendarCellItem2 else { return }
            setupCell(with: item)
        }
    }
    
    static func getCellHeight(for item: ModularTableViewItem) -> CGFloat {
        guard let item = item as? HomeCalendarCellItem2 else { return 0.0 }
...
extension HomeCalendarCellTable {
    fileprivate func setupCell(with item: HomeCalendarCellItem2) {
        events = item.events
...
extension HomeCalendarCellTable: UITableViewDataSource {
...
extension HomeCalendarCellTable: UITableViewDelegate {
