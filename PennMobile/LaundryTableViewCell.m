...
- (void)awakeFromNib {
    ...
    for(int i = 0; i < self.available_washers; i++) {
        ...
        [circleLayerGreen setPath:[[UIBezierPath bezierPathWithOvalInRect:CGRectMake(height*4/5 + i*height/6, height/2-height/16, height/8, height/8)] CGPath]];
    ...
    for(int i = self.available_washers; i < self.unavailable_washers+self.available_washers; i++) {
        ...
        [circleLayerRed setPath:[[UIBezierPath bezierPathWithOvalInRect:CGRectMake(height*4/5 + i*height/6, height/2-height/16, height/8, height/8)] CGPath]];
    ...
    for(int i = 0; i < self.available_dryers; i++) {
        CAShapeLayer *circleLayerGreen = [CAShapeLayer layer];
        [circleLayerGreen setFillColor:[UIColor greenColor].CGColor];
        [circleLayerGreen setPath:[[UIBezierPath bezierPathWithOvalInRect:CGRectMake(height*4/5 + i*height/6, height*3/4-height/16, height/8, height/8)] CGPath]];
        [[self layer] addSublayer:circleLayerGreen];
    }
    ...
    for(int i = self.available_dryers; i < self.unavailable_dryers+self.available_dryers; i++) {
        CAShapeLayer *circleLayerRed = [CAShapeLayer layer];
        [circleLayerRed setFillColor:[UIColor redColor].CGColor];
        [circleLayerRed setPath:[[UIBezierPath bezierPathWithOvalInRect:CGRectMake(height*4/5 + i*height/6, height*3/4-height/16, height/8, height/8)] CGPath]];
        [[self layer] addSublayer:circleLayerRed];
    }
}
